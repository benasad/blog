...
def deleteEntries(authToken, resourceId, col_names, csv_values):
        csv_value_string = ""
        for value in csv_values:
            csv_value_string+="{"
            csv_value_string+=""" "entry": """
        ...
        """
        headers = {'Content-Type': 'application/json', 'Accept': 'application/json'}
        r = requests.post('https://esm:8443/www/manager-service/rest/ActiveListService/deleteEntries', verify=installloc+'esm.crt', data=jsoninput, headers=headers)
        values = r.text
        return values

csv_col_count, csv_values = readCSV(installloc+'/todel.csv')
